...
// Check if employee needs to submit dependent persons request
export async function needsDependentRequest(employee: Employee | null): Promise<boolean> {
  if (!employee || employee.children_count === 0) return false
  
  try {
    const hasPendingRequest = await EmployeeRequestService.hasPendingDependentRequest(employee.id)
    return !hasPendingRequest
  } catch (error) {
    console.error('Error checking dependent request:', error)
    return false
  }
}

// Get missing fields including dependent request requirement
export async function getMissingFieldsExtended(employee: Employee | null): Promise<string[]> {
  const basicMissing = getMissingFields(employee)
  
  if (employee && employee.children_count > 0) {
    const needsRequest = await needsDependentRequest(employee)
    if (needsRequest) {
      basicMissing.push('Yêu cầu cập nhật số người phụ thuộc')
    }
  }
